...
func (c *appRepository) UpdateSpecificApp(objID primitive.ObjectID, owner string, ctxQuery map[string]interface{}, appLink, extension string, ctx context.Context) (bool, bool, bool, error) {
	...
	if err == nil {
		...
		if !teamUser.Permissions.Apps.Edit {
			...
			return false, false, false, errors.New("you don't have permission to edit apps")
		}
	...
	if teamUser.ID != primitive.NilObjectID {
		...
		if !hasAccess {
			...
			return false, false, false, errors.New("you don't have access to this app")
		}
	...
	if existingDoc.Err() == nil {
		...
		return true, isVelopack, isSparkle, nil
	} else {
		logrus.Debugf("Document does not exist with app_id: %s, version: %s, owner: %s",
			appMeta.ID.Hex(), ctxQuery["version"].(string), owner)
		return false, false, false, errors.New("app with this parameters doesn't exist")
	}
}
